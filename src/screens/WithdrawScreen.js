...
export default function WithdrawScreen() {
  ...
  const [date, setDate] = useState(new Date()); // stores both date & time
  ...
  const fetchAccounts = async () => {
    try {
      ...
      const validAccounts = all.filter((acc) => {
  ...
  const onSubmit = async () => {
    if (!amount || !description || !source?.name || !date) {
      Alert.alert('Missing Fields', 'Please fill amount, description, source, and date.');
      return;
    }
    ...
    try {
      ...
      const payload = {
        transactions: [
          {
            type: 'withdrawal',
            date: date.toISOString(), // contains both date & time
            amount,
            description,
      ...
      Alert.alert('Success', 'Withdrawal recorded!');
      resetForm();
    } catch (error) {
  ...
  const renderForm = () => (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={() => setShowTimePicker(true)}>
        <Text style={styles.dateInput}>{date.toLocaleTimeString()}</Text>
      </TouchableOpacity>
      {showTimePicker && (
        <DateTimePicker
          value={date}
      ...
      <TextInput
        ...
        onChangeText={(text) => {
          ...
          if (lastTag) {
            const matches = tagsList.filter((tag) => tag.toLowerCase().includes(lastTag));
      ...
      {filteredTags.length > 0 &&
        renderSuggestions(
          filteredTags.map((t, i) => ({ name: t, id: i.toString() })),
          (val) => {
